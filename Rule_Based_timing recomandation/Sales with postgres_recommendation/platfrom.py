...
def is_festival_near(selected_date):
    """
    Check if a shopping festival is today or within the next 7 days and return its name and date.
    """
    ...
    for festival, date_str in CHINA_SHOPPING_FESTIVALS.items():
        ...
        if days_diff == 0:
            festival_info.append(f"{festival} is today!")
        elif 0 < days_diff <= 7:
            festival_info.append(f"{festival} is near! ({festival_date.strftime('%Y-%m-%d')})")
...
def recommend_posting_time(product_category, date):
    ...
    # Check for festivals near or on the selected date
    festival_status = is_festival_near(date)

    return recommendations, festival_status, day_name
...
if st.button("Get Recommendations"):
    recommendations, festival_status, day_name = recommend_posting_time(selected_category, selected_date.strftime("%Y-%m-%d"))

    st.subheader(f"📌 Recommended Best Social Media Platforms & Times for '{selected_category}' Products on {selected_date}:")
    
    # Display whether the selected date is a weekday or weekend
    st.markdown(f"**Selected Date:** {selected_date} ({day_name})")

    # Display posting times for each platform
    if recommendations:
    ...
    if festival_status:
        st.subheader("⚡ Shopping Festivals ⚡")
        for festival in festival_status:
